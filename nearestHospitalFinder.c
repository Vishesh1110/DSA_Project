...
void platoxy_queries(int **arr, int **barr, int *parr, int *oarr, int n, edges edg[], int e, hospitals p[], int sub_choice)
{
    while (1)
    {
        ...
        if (ch == 1)
        {
            ...
            if (sub_choice == 2)
                printf("HOSPITALS \t no. of oxygen cylinders \n");
            if (sub_choice == 3)
                printf("HOSPITALS \t platelet units \n");
            for (int i = 1; i <= n; i++)
            {
                if (i == n || i == n - 1)
...
void blood_queries(int **arr, int **barr, int *parr, int *oarr, int n, edges edg[], int e, hospitals p[], int sub_choice)
{
    while (1)
    {
        ...
        if (ch == 1)
        {
            ...
            printf("\n\nRegisterd hospitals are : \n\n");
            printf("%s\n", p[0].name);
            printf("HOSPITALS \tA+\tA-\tB+\tB-\tAB+\tAB-\tO+\tO-\n");
            for (int i = 1; i <= n; i++)
            {
                if (i == n || i == n - 1)
                    printf("%s\t", p[i].name);
...
int **open_file(int **arr, int map_choice, int sub_choice)
{
    ...
    while (!feof(ptr))
    {
        ...
        // storing all the edges in the edges array (beneficial for kruskal's algo)
        edg[l].u = u;
        edg[l].v = v;
        edg[l].w = w;
        l = l + 1;
